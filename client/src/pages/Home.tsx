...
export default function Home() {
  ...
  return <div className="min-h-screen bg-paper text-ink">
    ...
    <main>
      ...
      <section className="border-b border-ink/10 bg-white py-16" id="automation"><div className="container"><div className="grid gap-10 lg:grid-cols-[.8fr_1.2fr]"><div><div className="font-mono text-[10px] uppercase tracking-[.16em] text-moss">The event-driven vision</div><h2 className="mt-3 font-display text-4xl font-semibold leading-tight">What if support research began the moment a company begins?</h2><p className="mt-4 max-w-xl text-[16px] leading-7 text-ink/65">The long-term PlainText.legal product does not wait for a founder to know what to search. A new business signal starts a focused research run automatically, translating the company’s public context into the programs, evidence, and next steps worth reviewing.</p><div className="mt-7 rounded-xl border border-amber-200 bg-amber-50 p-4 text-sm leading-relaxed text-amber-950/75"><div className="flex items-center gap-2 font-semibold text-amber-900"><ShieldCheck className="h-4 w-4" /> Automated, but permissioned</div><p className="mt-1">The production trigger requires a licensed domain-event partner or an authorized Utah registration export. PlainText.legal uses public company context and sends a report only through founder opt-in or a permitted partner workflow—it does not scrape WHOIS or harvest contact details.</p></div></div><div className="relative"><div className="absolute left-5 top-7 bottom-7 hidden w-px bg-moss/20 sm:block" />{[[<Globe2 className="h-4 w-4" />, "1. A company signal arrives", "A licensed new-domain event or authorized Utah company-registration event enters the workflow."], [<Building2 className="h-4 w-4" />, "2. Public context is interpreted", "The agent reads a supplied public domain or authorized company description, then builds a cautious startup profile."], [<SearchCheck className="h-4 w-4" />, "3. Support is researched and organized", "It searches live Grants.gov and USAspending data, layers SBIR/STTR context, and creates a Government Opportunity Map."], [<MailCheck className="h-4 w-4" />, "4. The founder gets a useful handoff", "The map is ready in PlainText.legal and can be delivered through an opted-in founder alert or permitted partner channel."]].map(([icon, title, detail]) => <div className="relative flex gap-4 py-4 first:pt-0 last:pb-0" key={String(title)}><div className="relative z-10 grid h-10 w-10 shrink-0 place-items-center rounded-xl bg-moss text-white">{icon}</div><div className="rounded-xl border border-ink/10 bg-paper p-4"><h3 className="font-semibold">{title}</h3><p className="mt-1 text-sm leading-relaxed text-ink/60">{detail}</p></div></div>)}</div></div><div className="mt-10 border-t border-ink/10 pt-5 text-sm leading-relaxed text-ink/55"><span className="font-semibold text-ink">Today:</span> founder-initiated scans run this same agent. <span className="font-semibold text-ink">Next:</span> approved data partners can trigger the same research automatically at company formation or domain launch.</div></div></section>
